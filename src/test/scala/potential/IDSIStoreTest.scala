...
import potential.indexBased.{IDSIStore, IDSMStore}

class IDSIStoreTest extends FunSuite {
   ...
   // creates the store
   val store = IDSIStore(variableSet1, values1)
   ...
   test("test of prune on potential of hepar2 potential") {
      ...
      val store = basePotential.convert(ValueStoreTypes.IDSISTORE).store.asInstanceOf[IDSIStore]
