...
import potential.valueBased.{VDGLStore, VDILMStore, VDISMStore}

class VDISMStoreTest extends FunSuite {
   ...
   // creates the store
   val store = VDISMStore(variableSet1, values1)
   ...
   test("test of prune on potential of hepar2 potential") {
      ...
      val store = basePotential.convert(ValueStoreTypes.VDISMSTORE).store.asInstanceOf[VDISMStore]
