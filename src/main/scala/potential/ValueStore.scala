...
object ValueStore extends Combiner{
   ...
   def createStore(store : ValueStore, values: List[Double]): ValueStore = {
      ...
      val result = store.kind match {
         ...
         case ValueStoreTypes.IDPIMMUT => IDPMStore(vars, values.toArray)
         case ValueStoreTypes.IDPMUT => IDPIStore(vars, values.toArray)
         case ValueStoreTypes.VDGSET => VDGSStore(vars, values.toArray)
         case ValueStoreTypes.VDGLIST => VDGLStore(vars, values.toArray)
         case ValueStoreTypes.VDILISTIMMUT => VDILIStore(vars, values.toArray)
         case ValueStoreTypes.VDILISTMUT => VDILMStore(vars, values.toArray)
         case ValueStoreTypes.VDISETIMMUT => VDISIStore(vars, values.toArray)
         case ValueStoreTypes.VDISETMUT => VDISMStore(vars, values.toArray)
         case ValueStoreTypes.IDSETIMMUT => IDSIStore(vars, values.toArray)
         case ValueStoreTypes.IDSETMUT => IDSMStore(vars, values.toArray)
         case ValueStoreTypes.IDMMUT => IDMMStore(vars, values.toArray)
      }
