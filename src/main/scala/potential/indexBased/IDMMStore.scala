 ...
 */
case class IDMMStore(variables: VariableSet,
                     indices: mutable.Map[Long, Long],
    ...
    */
   def addValueForRepresentation(value: Double, index: Long): IDMMStore = {
   ...
   /**
    * adds a new value and the sequence of indexes related to this
    * value
    * @param value new value to add
    * @param indexes indexes storing such value
    */
   def addValueForRepresentation(value : Double, indexes : ArrayBuffer[Long]): IDMMStore = {
      // gets the last index of values: this will be the position
      // for this new value
    ...
    * @param indexes indexes storing such value
    */
   def addNonCheckValueForRepresentation(value : Double, indexes : ArrayBuffer[Long]): IDMMStore = {
   ...
   override def split: List[ValueStore] = {
      (0L until values.length).map(index => {
         val indexes = indices.filter(_._2 == index).values.toArray
         IDMMStore(variables, values(index.toInt), indexes)
      }).toList
   }
   ...
   def getIndicesForValue(value: Double): List[Long] = {
      ...
      val result = if (valueIndex != -1) {
         indices.filter(entries => entries._2 == valueIndex).keys.toList
      }
      else {
         List()
      }
   ...
   override def getZerosProportion: Double = {
      ...
      // different from 0
      val numberOfIndices = indices.size
   ...
   override def getValuesProportions: List[Double] = {
      ...
      val counters = values.map(value => {
         indices.values.
            map(indexInValues =>
               values(indexInValues.toInt) == value).size
      })
   ...
   override def getSize: (Long, Long, Long) = {
      ...
      (indices.size, values.length, values.length)
   }
   ...
   def getMemorySize: Long = {
      ...
      val structuresSize = DataSizes.MAP + DataSizes.ARRAY
   ...
   override def merge(value1: Double, value2: Double): ValueDrivenStore = {
      ...
      // change values content by the one in newValues
      values.clear()
      values ++= newValues
...
object IDMMStore extends Combiner with Marginalizer {
    ...
    */
   def apply(variables: VariableSet, values: Array[Double]): IDMMStore = {
      ...
      val result = variables.variableList match {
         ...
         // empty store
         case Nil =>
            new IDMMStore(variables, mutable.Map[Long, Long](), ArrayBuffer(0))
         ...
         case _ =>
            ...
            val newStore = new IDMMStore(variables, mapIndices,
    ...
    */
   def apply(variables: VariableSet): IDMMStore = {
      // in any case creates an empty storage
      new IDMMStore(variables, mutable.Map[Long, Long](), ArrayBuffer())
   }

   /**
    * creates a store for a values and its indexes
    * @param variables variables for the store
    * @param value sing value for the store
    * @param indices indices where the value is assigned
    * @return
    */
   def apply(variables : VariableSet, value : Double, indices : Array[Long]) = {
   ...
   def combineAlt1(valst1: ValueStore, valst2: ValueStore): ValueStore = {
      ...
      // creates the result
      val result = IDMMStore(mapper.resultDomain)
   ...
   def combineAlt3(valst1: ValueStore, valst2: ValueStore): ValueStore = {
      ...
      // creates the result
      val result = new IDMMStore(mapper.resultDomain, mutable.Map[Long, Long](),
         ArrayBuffer())
   ...
   def combineAlt6(valst1: ValueStore, valst2: ValueStore): ValueStore = {
      ...
      // default value
      //println("potential.MapIMStore::combineAlt6 - starting triplets obtention")
      ...
      //println("potential.MapIMStore::combineAlt6 - starting indexes combination")
      ...
      //println("potential.MapIMStore::combineAlt6 - starting store creation")
      val result = IDMMStore(mapper.resultDomain)
   ...
   def combineAlt7(valst1: ValueStore, valst2: ValueStore): ValueStore = {
      ...
      // defa  ult value (addValue) and if the value is already
    ...
    * @return result of combination
    */
   def combineAlt8(valst1: ValueStore, valst2: ValueStore): ValueStore = {
      //println("potential.MapIMStore::combineAlt8 - potentials info: ")
      //println("valst1 card: " + valst1.variables.possibleValues + " dif values: " + valst1.getDifferentValues.length +
      //   " indexes: " + valst1.getIndices.length)
      //println("valst2 card: " + valst2.variables.possibleValues + " dif values: " + valst2.getDifferentValues.length +
      //   " indexes: " + valst2.getIndices.length)
      // creates a mapper object
      val mapper = CombineMapper(valst1.variables, valst2.variables)
      ...
      println("   comb loop on: " + mapper.resultDomain.possibleValues)
      while (index < mapper.resultDomain.possibleValues) {
         // produces the pair
         val indexes: (Long, Long) = mapper.mapIndices2(index)
         //println("index in result: " + index + " pairs of indexes: " + indexes)

         // produces the value
         val value=Util.roundNumber(valst1.getValue(indexes._1) * valst2.getValue(indexes._2))
         result.addValue(value, index)
   ...
   def marginalizeAlt3(valst : ValueStore, variable : Variable) : ValueStore = {
      ...
      //println("potential.MapIMStore::marginalizeAlt3 - grouping indexes")
      ...
      //println("potential.MapIMStore::marginalizeAlt3 - computing values")
      ...
      // creates the result as final step
      //println("potential.MapIMStore::marginalizeAlt3 - making store")
      val result = IDMMStore(mapper.resultDomain)
    ...
    * @return result of marginalization
    */
   def marginalizeAlt4(valst: ValueStore, variable: Variable): ValueStore = {
      // creates a mapper object for this operation
      val mapper = MarginalizeMapper(variable, valst.variables)

      // for each index in result gets the corresponding
      // values
      val content = ArrayBuffer[Double]()

      // iterates on result
      var index = 0L
      //println("   marg loop on: " + mapper.resultDomain.possibleValues)
      while (index < mapper.resultDomain.possibleValues) {
         //if(index % 1000000 == 0) println("    marginalizeAlt4 - index: " + index)
         val value = mapper.mapIndexFromResultToSource(index).
            par.map(valst.getValue).sum
         content += Util.roundNumber(value)
      ...
      // creates the result as final step
      val result = IDMMStore(mapper.resultDomain, content.toArray)
